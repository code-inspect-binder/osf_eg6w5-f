...
# import data
dat <- read.csv("experiment1c_data.csv")
str(dat)
names(dat)
...
dat.long <- melt(dat, id.vars=c("subject", "condition"), 
                 measure.vars=c("pity.target", "envy.target", "pityAD.decoy", "envyAD.decoy", "pityC.decoy", "envyC.decoy"), na.rm = FALSE)
...
# plot
# pdf("experiment1c_figure.pdf", width = 8.5, height = 8.5)
...
text(x = 3, y = 6.45, "Experiment 1c", cex = 2.2, adj = c(1, NA))
...
text(2.15, (((forfig[forfig$target == 'pity.target' & forfig$condition == 'EnvyAD', "emmean"]) +
               (forfig[forfig$target == 'pity.target' & forfig$condition == 'Baseline', "emmean"] - 0.03)) /2),
     "*", cex = 2, adj = c(0, NA))
segments(2.89, forfig[forfig$target == 'pity.target' & forfig$condition == 'EnvyC', "emmean"] , 
         2.89, forfig[forfig$target == 'pity.target' & forfig$condition == 'Baseline', "emmean"] - 0.03, lwd = 2)
text(2.84, (((forfig[forfig$target == 'pity.target' & forfig$condition == 'EnvyC', "emmean"]) +
               (forfig[forfig$target == 'pity.target' & forfig$condition == 'Baseline', "emmean"] - 0.03)) /2),
     "**", cex = 2, adj = c(1, NA))
par(op)
# dev.off()