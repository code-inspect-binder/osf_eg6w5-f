...
# import data
dat <- read.csv("experiment1d_data.csv")
str(dat)
names(dat)
...
dat.long <- melt(dat, id.vars=c("subject","condition"), measure.vars=c("pity.target", "envy.target", "envyC.decoy.m", "envyC.decoy.f")) 
...
anova(fitlmer1, fitlmer2) # hypothesis-driven model is marginally better than reduced model
...
round(s[s$target == 'envy.target' & s$condition == 'EnvyC-F', "emmean"] - s[s$target == 'pity.target' & s$condition == 'EnvyC-F', "emmean"],3)
round(s[s$target == 'envy.target' & s$condition == 'EnvyC-M', "emmean"] - s[s$target == 'pity.target' & s$condition == 'EnvyC-M', "emmean"],3)
...
# plot
# pdf("experiment1d_figure.pdf", width = 8.5, height = 8.5)
op <- par(mar=c(5,6,4,2))
plot(1, ylim = c(4.4,6.5), xlim = c(0,4), ylab = "", xlab = "", xaxt = "n", yaxt = "n", cex.axis = 2.5, ann = FALSE)
mgp.axis(side = 1, at = c(1, 3), labels = c("EnvyC Female", "EnvyC Male"), cex.axis = 2, line = 0)
mgp.axis(side = 2, cex.axis = 2, line = 0, las = 1)
title(ylab = "Likelihood of Hiring", line=3.5, cex.lab=2)
title(xlab = "Condition", line=3, cex.lab=2)
polygon(c(-1,-1,5,5), c(forfig[forfig$target == 'pity.target' & forfig$condition == 'Baseline', "lower.CL"],
...
polygon(c(-1,-1,5,5), c(forfig[forfig$target == 'envy.target' & forfig$condition == 'Baseline', "lower.CL"],
...
arrows(c(1,3), forfig[forfig$target == 'pity.target' & forfig$condition %in% c('EnvyC-F', 'EnvyC-M'), "lower.CL"],
       c(1,3), forfig[forfig$target == 'pity.target' & forfig$condition %in% c('EnvyC-F', 'EnvyC-M'), "upper.CL"],
       length=0, angle=90, code=3, col = alpha(col = "firebrick1", 0.8), lwd = 3)
arrows(c(1,3), forfig[forfig$target == 'envy.target' & forfig$condition %in% c('EnvyC-F', 'EnvyC-M'), "lower.CL"],
       c(1,3), forfig[forfig$target == 'envy.target' & forfig$condition %in% c('EnvyC-F', 'EnvyC-M'), "upper.CL"],
       length=0, angle=90, code=3, col = alpha(col = "royalblue1", 0.8), lwd = 3)
points(x= 1, y = forfig[forfig$target == 'pity.target' & forfig$condition == 'EnvyC-F', "emmean"], pch = 16, col = alpha(col = "firebrick1", 1), cex = 3)
points(x= 1, y = forfig[forfig$target == 'envy.target' & forfig$condition == 'EnvyC-F', "emmean"], pch = 16, col = alpha(col = "royalblue1", 1), cex = 3)
points(x= 3, y = forfig[forfig$target == 'pity.target' & forfig$condition == 'EnvyC-M', "emmean"], pch = 16, col = alpha(col = "firebrick1", 1), cex = 3)
points(x= 3, y = forfig[forfig$target == 'envy.target' & forfig$condition == 'EnvyC-M', "emmean"], pch = 16, col = alpha(col = "royalblue1", 1), cex = 3)
text(2, forfig[forfig$target == 'pity.target' & forfig$condition == 'Baseline', "emmean"] - 0.08, "Pity Baseline", cex = 2)
text(2, forfig[forfig$target == 'envy.target' & forfig$condition == 'Baseline', "emmean"] - 0.08, "Envy Baseline", cex = 2)
points(x = 0, y = 6.45, pch = 16, col = alpha(col = "firebrick1"), cex = 2)
text(x = 0.2, y = 6.45, "Pity Candidate", cex = 2, adj = c(0,NA))
points(x = 0, y = 6.35, pch = 16, col = alpha(col = "royalblue1"), cex = 2)
text(x = 0.2, y = 6.35, "Envy Candidate", cex = 2, adj = c(0,NA))
text(x = 4, y = 6.45, "Experiment 1d", cex = 2.2, adj = c(1,NA))
segments(1.15, forfig[forfig$target == 'pity.target' & forfig$condition == 'EnvyC-F', "emmean"], 
         1.15, forfig[forfig$target == 'pity.target' & forfig$condition == 'Baseline', "emmean"] - 0.03, lwd = 2)
text(1.2, (((forfig[forfig$target == 'pity.target' & forfig$condition == 'EnvyC-F', "emmean"]) +
